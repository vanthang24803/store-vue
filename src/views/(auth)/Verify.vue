...
watchEffect(() => {
    try {
        loading.value = true;
        auth.verifyAccount(userId.value, encodeURIComponent(token.value.replaceAll(" ", "+")))
        active.value = true;
    } catch (error) {
        console.log(error);
    } finally {
...
<template>
    <Modal>
        ...
        <div class="py-2 flex items-center justify-center flex-col space-y-6" v-show="active">
            ...
            <div class="flex items-center justify-center space-x-6 mt-8">
                <Button variant="primary" class="w-[150px]" size="default" @click="router.push({ path: '/' })">
                    Trở về trang chủ
                </Button>
            </div>
        </div>
