...
<template>
  <main class="my-12 py-6 px-4 bg-neutral-100/90 min-h-screen">
    <div class="md:container">
      ...
      <div
        class="flex lg:flex-row flex-col justify-between lg:space-x-4 w-full"
      >
        <div
          class="lg:w-2/3 w-full bg-white rounded p-4 flex flex-col space-y-3"
        >
          ...
          <div class="w-full rounded-md border border-neutral-200 p-2">
            <ScrollArea class="h-[50vh]">
              <div
                class="flex flex-col space-y-4 my-4 w-full"
                v-for="(item, index) in cart.items"
                :key="index"
              >
                <div
                  class="flex flex-col space-y-2 text-sm hover:cursor-pointer"
                >
                    ...
                    <div class="flex flex-col w-full">
                      ...
                      <div class="flex items-center justify-between my-2">
                        ...
                        <div class="flex items-center space-x-2 md:mx-4">
                          ...
                          <span
                            v-show="item.product.options[0].sale > 0"
          ...
          <span class="text-sm font-bold py-3"> Ghi chú đơn hàng </span>

          <Textarea
            role="60"
            cols="10"
            class="w-full rounded-md border border-neutral-200 p-4"
          />
        </div>
        <div
          class="lg:w-1/3 w-full lg:h-[38vh] bg-white rounded p-4 flex flex-col space-y-3"
        >
          <h1 class="text-xl font-bold">Giỏ hàng của bạn</h1>
          <Separator />
          <div class="flex items-center justify-between">
            <h2 class="text-base font-bold">Tổng tiền:</h2>
            <p class="text-2xl font-bold text-[#ff0000]">
              {{ cart.totalPrice }}
            </p>
          </div>
          <Separator />
          ...
          <Button
            variant="destructive"
            class="bg-[#ff0000] uppercase font-semibold"
