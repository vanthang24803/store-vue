...
const fetchOrder = async () => {
  const response = await _http.get(`/api/order/${auth.user?.id}/user`);

  if (response.status == 200) {
    totalOrder.value = response.data.filter(
      (c) => c.status != "SUCCESS",
    ).length;
  }
};
...
<template>
  <nav
    className="fixed top-0 w-full h-14 border-b px-6 shadow-sm bg-white flex items-center z-50 text-sm"
  >
    <div
      className="md:max-w-screen-xl mx-auto flex items-center w-full justify-between"
    >
      ...
      <div class="flex items-center space-x-4 hover:cursor-pointer">
        <DropdownMenu v-if="auth.isLogin">
          ...
          <DropdownMenuContent class="w-56">
            <DropdownMenuLabel
              >{{ auth.user.name }}
              <b v-show="isAdmin" className="uppercase text-destructive">
                - Admin
              </b>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            ...
            <div class="flex flex-col text-[12px] p-2 space-y-1">
              ...
              <div class="w-full rounded h-[3px] bg-neutral-200">
                <div
                  class="rounded h-[3px]"
                  :class="percentage > 0 ? 'bg-green-500' : 'bg-neutral-100'"
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem
                @click="router.push({ path: `/profile/${auth.user.id}` })"
              >
              ...
              <DropdownMenuItem
                @click="
                  router.push({ path: `/profile/${auth.user.id}/orders` })
                "
              >
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <div
                    class="flex items-center justify-center w-5 h-5 rounded-full text-white bg-red-500 text-sm animate-bounce"
                    v-show="totalOrder > 0"
                  >
                    {{ totalOrder }}
                  </div>
              ...
              <DropdownMenuItem
                @click="
                  router.push({ path: `/profile/${auth.user.id}/address` })
                "
              >
