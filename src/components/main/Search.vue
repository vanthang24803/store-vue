...
import { handlerSearch } from "@/api/search";

const content = ref("");

const { data: product } = useQuery({
  queryKey: ["search", content],
  queryFn: () => handlerSearch(content),
});
</script>
...
<template>
  <div class="relative md:block hidden dark:border-white group" ref="ref">
    <Input
      class="lg:w-[500px] md:w-[300px] h-10 font-medium"
      v-model="content"
      placeholder="Tìm kiếm sản phẩm..."
    />

    <RouterLink
      :to="`/search?product=${content}`"
    ...
    >
      ...
      <div
        v-if="product"
        class="w-full h-auto my-2 text-sm flex flex-col space-y-2"
      >
        <div class="flex flex-col space-y-2">
          <div
            v-for="(item, index) in product.slice(0, 5)"
            :key="index"
          ...
          >
            ...
            >
              <div class="flex flex-col space-y-1">
                <span class="hover:text-[#65b10d] font-medium">{{
                  item.name
                }}</span>
