...
<template>
  <div class="my-6 md:my-8 w-full bg-white rounded-md p-6">
    <RouterLink to="/blogs" class="flex items-center justify-between">
      ...
      <div
        class="flex items-center text-sm space-x-1 text-sky-500 font-semibold"
      >
        <p>Xem thêm</p>
    ...
    <div>
      ...
      <div
        v-else
        class="flex md:flex-row flex-col my-4 space-y-4 md:space-y-0 md:space-x-4"
      >
        <div
          v-if="firstBlog"
          class="md:basis-1/2 w-full flex flex-col hover:cursor-pointer space-y-2"
          @click="
            router.push(`/blogs/${generateSlug(firstBlog.title, firstBlog.id)}`)
          "
        >
          <img
        ...
        <div
          class="md:basis-1/2 w-full flex flex-col space-y-4 lg:space-y-6"
          v-if="restBlog.length > 0"
        >
          <div
            class="flex space-x-4 hover:cursor-pointer"
            v-for="item in restBlog"
            :key="item.id"
