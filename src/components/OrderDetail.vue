...
<template>
    <div class="hidden lg:block w-full  bg-neutral-50 p-8">
        <ScrollArea class="max-h-svh">
            <div class="flex flex-col space-y-3 " v-for="(item, index) in cart.items" :key="index">
                <RouterLink :to="`/product/${item.product.id}`" target="_blank" class="flex items-start space-x-2 my-2">
                    <img :src="item.product.thumbnail" :alt="item.product.name" class="w-[15%] relative" />
                    <div class="flex items-center justify-between w-full">
                        <div class="flex flex-col text-sm">
                            ...
                            <span class="mt-2 text-neutral-700">
                                x{{ item.quantity }}
                            </span>
                        </div>
        ...
        <div class="my-4 flex flex-col space-y-6  w-full">
            ...
            <div class="flex flex-col space-y-2">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span v-if="ship" class="text-sm font-medium"> {{ price(codPrice) }}₫ </span>
                    <span v-else> - </span>
                </div>
            </div>
            <Separator />
        ...
        <div class="flex items-center justify-between w-full">
            ...
            <div class="flex items-center space-x-2">
                <span>VND</span>
                <p v-if="ship" class="text-xl font-semibold">
                    {{ price(priceCod) }}₫
                </p>
